    ...
    if choice == 'Sign up':
        ...
        if submit:
            if email and password is not None:
              ...
              st.info('Login via login drop down selectbox')
            else:
             st.sidebar.error("Enter all details")
    elif choice == 'Login':
    ...
    if app_options == 'Movie based':
        ...
        if movie_select == '--Select--':
            ...
            for movie, link in table:
                ...
                st.markdown("---")
                with st.container():
                    name_column, link_column = st.columns((1, 2))
                    with name_column:
                        st.write(movie)
                    with link_column:
                      st.write("Follow link to see details")
                      st.markdown(link)
                        # if st.button("Read details", key=movie):
